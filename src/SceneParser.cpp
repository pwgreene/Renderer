...
SceneParser::parseSpotLight()
{
    ...
    while (true) {
        ...
        if (!strcmp(token, "position")) {
            ...
            color = readVector3f();
        } else if (!strcmp(token, "falloffAngle")) {
            falloff = readFloat();
        } else if (!strcmp(token, "hotspotAngle")) {
...
LuminousMaterial *
SceneParser::parseLuminousMaterial()
{
    char token[MAX_PARSER_TOKEN_LENGTH];
    char filename[MAX_PARSER_TOKEN_LENGTH];
    filename[0] = 0;
    ...
    LuminousMaterial *answer = new LuminousMaterial(diffuseColor, specularColor, shininess, refractionIndex, lightColor, falloff);
    areaLights.push_back(answer);
...
Rectangle *
SceneParser::parseRectangle()
{
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token); assert(!strcmp(token, "{"));
    getToken(token); assert(!strcmp(token, "normal"));
    Vector3f normal = readVector3f();
    getToken(token); assert(!strcmp(token, "vecmin"));
    Vector3f vecmin = readVector3f();
    getToken(token); assert(!strcmp(token, "vecmax"));
    Vector3f vecmax = readVector3f();
    getToken(token); assert(!strcmp(token, "}"));
    assert(_current_material != NULL);
    return new Rectangle(normal,vecmin,vecmax,_current_material);
}
