        ...
        for (int y = 0; y < h; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                if (_args.photonmapping) {
                    ...
                    color += causticMap->gatherAndFilter(gatherSpot, bpq, dSq, k, true)/totalPhotons; //clears bpq
...
//        add to shadow map
        ...
        if (randVal < m->getPAbsorption()) {
            ...
            if (m->getIsDiffracting()) {
                float nColors = 5;
                //diffraction for different wavelengths (rgb doesn't match quite up with physics...)
                Photon pRed;
                pRed.color =   p.color*Vector3f(1.0, 0.0, 0.0)/nColors; pRed.hitSpecular = true;
//                Photon pOrange;
//                pOrange.color =p.color*Vector3f(1.0, 0.5, 0.0)/3.0; pOrange.hitSpecular = true;
                Photon pYellow;
                pYellow.color =p.color*Vector3f(1.0, 1.0, 0.0)/nColors; pYellow.hitSpecular = true;
                Photon pGreen;
                pGreen.color = p.color*Vector3f(0.0, 1.0, 0.0)/nColors; pGreen.hitSpecular = true;
                Photon pBlue;
                pBlue.color =  p.color*Vector3f(0.0, 0.0, 1.0)/nColors; pBlue.hitSpecular = true;
                Photon pViolet;
                pViolet.color=p.color*Vector3f(0.5, 0.0, 1.0)/nColors; pViolet.hitSpecular = true;
                sendPhoton(r.refract(h.getNormal(),
                                     r.isInsideObject ? m->getRefractionIndex() : 1.0,
                                     r.isInsideObject ? 1.0 : m->getRefractionIndex(),
                                     r.pointAtParameter(h.getT())), pRed);
//                sendPhoton(r.refract(h.getNormal(),
//                                     r.isInsideObject ? m->getRefractionIndex()+.05 : 1.0,
//                                     r.isInsideObject ? 1.0 : m->getRefractionIndex(),
//                                     r.pointAtParameter(h.getT())), pOrange);
                sendPhoton(r.refract(h.getNormal(),
                                     r.isInsideObject ? m->getRefractionIndex()+.02 : 1.0,
                                     r.isInsideObject ? 1.0 : m->getRefractionIndex(),
                                     r.pointAtParameter(h.getT())), pYellow);
                sendPhoton(r.refract(h.getNormal(),
                                     r.isInsideObject ? m->getRefractionIndex()+.03 : 1.0,
                                     r.isInsideObject ? 1.0 : m->getRefractionIndex(),
                                     r.pointAtParameter(h.getT())), pGreen);
                sendPhoton(r.refract(h.getNormal(),
                                     r.isInsideObject ? m->getRefractionIndex()+.06 : 1.0,
                                     r.isInsideObject ? 1.0 : m->getRefractionIndex(),
                                     r.pointAtParameter(h.getT())), pBlue);
                return sendPhoton(r.refract(h.getNormal(),
                                     r.isInsideObject ? m->getRefractionIndex()+.08 : 1.0,
                                     r.isInsideObject ? 1.0 : m->getRefractionIndex(),
                                     r.pointAtParameter(h.getT())), pViolet);
            } else {
                return sendPhoton(r.refract(h.getNormal(),
                                        r.isInsideObject ? m->getRefractionIndex() : 1.0,
                                        r.isInsideObject ? 1.0 : m->getRefractionIndex(),
    ...
    if (_scene.getGroup()->intersect(r, tmin, h)) {
        ...
        if (bounces > 0) {
            ...
            if (h.getMaterial()->getRefractionIndex() > 0) {
                ...
                if (h.getMaterial()->getFadeDistance() > 0) {
                    float distanceThroughObject = r.isInsideObject ? h.getT() : 0;
                    fadeFactor = h.getMaterial()->getFadeDistance() - distanceThroughObject > 0 ?
                                 h.getMaterial()->getFadeDistance() - distanceThroughObject : 0;
                }
                color += fadeFactor * h.getMaterial()->getSpecularColor(p) * traceRay(refractedRay, epsilon, bounces-1, refractionHit);
